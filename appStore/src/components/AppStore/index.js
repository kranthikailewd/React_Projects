...
class AppStore extends Component {
  state = {
    searchTerm: '',
    appsDisplayList: appsList.filter(each => each.category === 'SOCIAL'),
  ...
  onClickTabChange = id => {
    const updatedAppsList = appsList.filter(each => each.category === id)
    this.setState({appsDisplayList: updatedAppsList, activeTabId: id})
  }

  render() {
    const {searchTerm, appsDisplayList, activeTabId} = this.state

    const filteredSearchResults = appsDisplayList.filter(each =>
      each.appName.toLowerCase().includes(searchTerm.toLowerCase()),
    )
      ...
      <div className="main_container">
        ...
        <ul className="tabs_container">
          {tabsList.map(each => (
            <TabItem
              key={each.tabId}
              onClickTabChange={this.onClickTabChange}
        ...
        <ul className="apps_container">
          {filteredSearchResults.map(each => (
